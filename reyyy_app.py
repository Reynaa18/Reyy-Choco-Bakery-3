...
if pilihan == "Tentang Toko":
    ...
    if not st.session_state.pesanan:
        ...
        if metode == "Non-tunai":
            ...
            # Ganti QR Code dengan link ke DANA menggunakan nomor HP
            dana_link = f"https://link.dana.id/qr/082119761060"
            qr = qrcode.make(dana_link)
            buf = BytesIO()
            qr.save(buf)
            st.image(buf.getvalue(), caption="Scan untuk bayar via DANA")
